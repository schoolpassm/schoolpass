...
import { formatKRW, PRODUCT_LABEL } from "@/lib/commission";
      ...
      <table className="w-full min-w-[1150px] text-sm">
        <thead>
          <tr className="border-b border-surface-border bg-surface-muted text-left text-xs text-ink-500">
            ...
            <th className="px-4 py-3 font-medium">산정방식</th>
            <th className="px-4 py-3 font-medium">수수료</th>
        ...
        <tbody>
          {contracts.map((c) => (
            <tr key={c.id} className="border-b border-surface-border last:border-0 hover:bg-surface-muted/60">
              ...
              <td className="px-4 py-3 text-ink-500">{PRODUCT_LABEL[c.product] ?? "-"}</td>
              ...
              <td className="px-4 py-3 text-ink-500">
                {c.calcMethod === "unit" ? `단위판매 (${c.unitCount ?? 0}대)` : "사업예산"}
              </td>
              <td className="px-4 py-3 font-semibold text-primary-600">{formatKRW(c.commission.totalCommission)}</td>
          ...
          {contracts.length === 0 && (
            <tr>
              <td colSpan={10} className="px-4 py-16 text-center text-sm text-ink-300">
                등록된 계약이 없습니다.
              </td>
            </tr>
          )}
